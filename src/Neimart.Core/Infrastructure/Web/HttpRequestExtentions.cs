...
namespace Neimart.Core.Infrastructure.Web
{
    public static class HttpRequestExtentions
    {
        ...
        public static string RefererUrl(this HttpRequest request)
        {
            var referrerUrl = request.Headers["Referer"].FirstOrDefault();
            return referrerUrl;
        }
            ...
            public static bool IsPostBack(this HttpRequest request)
        {
            ...
            bool isSameUrl = string.Equals(currentUrl, referrerUrl, StringComparison.InvariantCultureIgnoreCase);

            return isPost && isSameUrl;
        }

        public static bool IsCallBack(this HttpRequest request)
        {
            var currentUrl = UriHelper.BuildAbsolute(request.Scheme, request.Host, request.PathBase, request.Path, request.QueryString);
            var referrerUrl = request.Headers["Alt-Referer"].FirstOrDefault();
        ...
        public static bool IsMobile(this HttpRequest request)
        {
            ...
            // source: https://stackoverflow.com/questions/13086856/mobile-device-detection-in-asp-net/13086894#13086894
            var MobileCheck = new Regex(@"(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);
